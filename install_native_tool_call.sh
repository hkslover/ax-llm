...
# ax-llm native tool call installer
#
# 行为概述（按优先级）：
...
#
# 额外说明：
# - “下载 native tool call 分支的代码”：默认从 hkslover fork 克隆并固定使用
#   feat/native-tool-call 分支；也可通过环境变量覆盖：REPO_URL、BRANCH。
# - 该分支的 .gitmodules 已指向 native tool call 所需的 third-party fork；
#   递归子模块更新会使用该分支记录的 URL 和 commit。
# - 安装到 /usr/bin 需要 root 权限，脚本会自动通过 sudo 提权（若存在）。

PROJECT_NAME="ax-llm"
AX650_BIN="axllm"
AXCL_BIN="axllm"
DEFAULT_NATIVE_REPO_URL="https://github.com/hkslover/ax-llm.git"
DEFAULT_NATIVE_BRANCH="feat/native-tool-call"
...
get_repo_and_branch() {
  ...
  # native tool call only exists on the fork by default. Prefer remote.hkslover
  # over local origin, because origin may still be AXERA-TECH/ax-llm.
  if [ -z "${REPO_URL:-}" ]; then
    REPO_URL=$(git -C "$here" config --get remote.hkslover.url 2>/dev/null || true)
  fi
  if [ -z "${REPO_URL}" ] && [ ! -f "${here}/CMakeLists.txt" ]; then
    # fallback for curl | bash usage
    REPO_URL="${DEFAULT_NATIVE_REPO_URL}"
  fi
  if [ -z "${REPO_URL}" ] && [ -d "${here}/.git" ]; then
    REPO_URL="${DEFAULT_NATIVE_REPO_URL}"
  fi
  # 强制默认分支为 feat/native-tool-call，允许通过环境变量 BRANCH 覆盖
  : "${BRANCH:=${DEFAULT_NATIVE_BRANCH}}"
}
...
main() {
  ...
  echo "Native defaults: REPO_URL=${DEFAULT_NATIVE_REPO_URL} BRANCH=${DEFAULT_NATIVE_BRANCH}" >&2
  exit 2
}
